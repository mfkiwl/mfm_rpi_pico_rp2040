...
void loop()
{
  ...
  if (Auswertung == true)
  {
    ...
    Check = false;
    if ((fMess > 45000) && (fMess < 55000) && (FA == 50))
      Check = true;
    if ((fMess > 55000) && (fMess < 65000) && (FA == 60))
      Check = true;
...
word Diff_filter(word filter_wert)
{
  ...
  {
    if (Diff > 0)
      filter_wert = Cachewert + DiffMAX; // Änderung (positive Steigung) max
    if (Diff < 0)
      filter_wert = Cachewert - DiffMAX; // Änderung (negative Steigung) max
  }
...
{
  // Messroutine
  // if (Triggersperr < TCNT1) 
  if (Triggersperr) // 1. Überprüfung: Sperrzeit eingehalten ?
  {
